...
module LlvmMangler_V1 ( llvmFixupAsm ) where
...
llvmFixupAsm f1 f2 = do
    ...
    allTables h (BS.breakSubstring dataStart asm)
    return ()
...
oneTable h (fStr, iStr) =
    let last' xs = if (null xs) then 0 else last xs
        ...
        -- get the function
        (bl, al)          = BS.breakSubstring functionSuf fStr
        ...
        (bit, itable)     = BS.breakSubstring ilabel iStr
