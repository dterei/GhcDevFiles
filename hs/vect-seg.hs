...
multListTerei :: Int -> Vector Vec4 -> Vector Vec4
multListTerei !count !src
   | count <= 0    = src
   | otherwise     = multListTerei (count-1) $ V.map (\v -> add (mult v m) a) src

main = do
   print $ Data.Vector.Storable.sum
         $ Data.Vector.Storable.map vsum
         $ multListTerei repCount
         $ Data.Vector.Storable.replicate arraySize (Vec4 1 1 1 1)
